...
Use py_repl for multi-step tasks with a persistent work queue. The supervisor loop has a **checkpoint** after each phase that evaluates actual project state and adapts the plan — retrying, splitting oversized items, or skipping when appropriate.
...
def run_phase(item, q):
    """Worker -> verifier -> merge. Returns (merged: bool, result)."""
    w = f"worker-{item['id']}"
    ...
    worker = tau.thread(w, task, tools=["full"],
    ...
    verifier = tau.thread(v,
        ...
        episodes=[w], timeout=timeout)

    if verifier.completed:
        merged = tau.merge(v)
        return bool(merged), verifier
...
for _ in range(max_iter):
    ...
    tau.log(f"Starting: {item['id']}")
    save_queue(queue)

    merged, result = run_phase(item, queue)
...
- **Checkpoint evaluates real state.** After each phase, `cargo build` + `test_runner.py --summary` reveals what actually works, not what the plan says should work.
...
- **State lives on disk.** The workqueue JSON is the source of truth. Attempts history survives session restarts. Each decision is logged.
