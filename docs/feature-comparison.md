...
Feature-level comparison across 7 harnesses: **tau**, **pi-mono**, **oh-my-pi**, **pi_agent_rust**, **codex**, **crush**, and **opencode**.
...
| Tool | tau | pi-mono | oh-my-pi | pi_agent_rust | codex | crush | opencode |
|------|-----|---------|----------|---------------|-------|-------|----------|
| bash/shell | yes | yes | yes | yes | yes | yes | yes |
| file read | yes | yes | yes | yes | yes | yes (view) | yes |
| file write | yes | yes | yes | yes | via shell | yes | yes |
| file edit (exact match) | yes | yes | yes | yes | -- | yes | yes |
| multi-edit | -- | -- | -- | -- | -- | yes | yes |
| hashline edit | yes | -- | yes (invented it) | yes (default) | -- | -- | -- |
| apply_patch (unified diff) | -- | -- | yes (patch mode) | -- | yes (primary) | -- | yes (GPT models) |
| grep/rg | yes | yes | yes | yes | yes | yes | yes |
| glob/find | yes | yes | yes | yes | -- | yes | yes |
| ls | -- | yes | -- | yes | yes | yes | -- |
| web fetch | -- | -- | yes | -- | -- | yes | yes |
| web search | -- | -- | yes (10 providers) | -- | yes (Responses API) | yes | yes (Exa) |
...
| ssh | -- | -- | yes | -- | -- | -- | -- |
| calculator | -- | -- | yes | -- | -- | -- | -- |
| todo/plan tracking | -- | -- | yes | -- | yes (update_plan) | yes | yes |
| sub-agent spawn | -- | ext example | yes (8 types) | -- | yes (spawn/wait/send) | -- | yes |
| batch parallel tools | -- | -- | -- | -- | -- | -- | yes (25 concurrent) |
| download | -- | -- | -- | -- | -- | yes | -- |
| sourcegraph | -- | -- | -- | -- | -- | yes | -- |
| code search | -- | -- | -- | -- | -- | -- | yes (Exa) |
| checkpoint/rewind | -- | -- | yes | -- | -- | -- | -- |
| cancel background job | -- | -- | yes | -- | -- | yes | -- |
| artifacts | -- | -- | yes | -- | yes | -- | -- |
| request user input | -- | -- | yes (ask) | -- | yes | -- | -- |
| MCP tools (dynamic) | -- | -- | yes | stub | yes | yes | yes |
| custom tools (extensions) | -- | yes | yes | yes | via MCP/plugins | -- | yes |

**Tool count (built-in)**: tau 6 | pi-mono 7 | oh-my-pi ~25 | pi_agent_rust 8 | codex ~16 | crush ~16 | opencode ~15

---

## Edit Strategy

| Feature | tau | pi-mono | oh-my-pi | pi_agent_rust | codex | crush | opencode |
|---------|-----|---------|----------|---------------|-------|-------|----------|
| Exact string replace | yes | yes | yes | yes | -- | yes | yes |
...
| Feature | tau | pi-mono | oh-my-pi | pi_agent_rust | codex | crush | opencode |
|---------|-----|---------|----------|---------------|-------|-------|----------|
| Auto-compaction | -- | yes | yes | yes | yes | yes | yes |
| Manual /compact | -- | yes | yes | yes | yes | yes | yes |
| LLM-based summarization | -- | yes | yes | yes | yes | yes | yes |
...
| Thinking level control | -- | yes | yes | yes | yes (reasoning effort) | yes | -- |
...
| Feature | tau | pi-mono | oh-my-pi | pi_agent_rust | codex | crush | opencode |
|---------|-----|---------|----------|---------------|-------|-------|----------|
| Sub-agent spawning | -- | ext example | yes (8 types) | -- | yes (spawn/wait/send/resume/close) | -- | yes |
| Max concurrent sub-agents | -- | -- | 32 | -- | -- | -- | -- |
| Background async jobs | -- | -- | yes (100 max) | -- | -- | -- | -- |
...
| Plan→build agent switch | -- | -- | -- | -- | yes (/plan) | -- | yes |
...
| Feature | tau | pi-mono | oh-my-pi | pi_agent_rust | codex | crush | opencode |
|---------|-----|---------|----------|---------------|-------|-------|----------|
| Per-tool permissions | -- | ext example | yes | yes (capability policy) | yes | yes | yes |
| Allow once / always | -- | -- | -- | yes (with expiry) | -- | -- | yes |
...
| Plan mode (read-only) | -- | ext example | yes | -- | yes | -- | yes |
...
| Feature | tau | pi-mono | oh-my-pi | pi_agent_rust | codex | crush | opencode |
|---------|-----|---------|----------|---------------|-------|-------|----------|
| Persistence format | JSONL | JSONL (v3) | JSONL | JSONL (v3) + segment store + SQLite | SQLite | SQLite | SQLite |
| Session resume | yes | yes | yes | yes | yes | yes | yes |
| Session picker (fuzzy) | -- | yes | -- | yes | -- | yes | yes |
| Branch/fork tree | -- | yes | yes | yes | yes (fork) | -- | -- |
| Session naming | -- | yes | yes | yes | yes (rename) | -- | -- |
| Session sharing | -- | yes (gist) | -- | yes (gist) | -- | yes | yes |
| HTML export | -- | yes | -- | yes | -- | -- | -- |
| Headless/print mode | yes | yes | -- | yes | yes (exec) | -- | yes |
| RPC mode | -- | yes | -- | yes | -- | -- | -- |
| App server (HTTP) | -- | -- | -- | -- | yes | -- | yes |
...
| OpenAI | yes | yes | yes | yes | yes (Responses API) | yes | yes |
...
| Custom model config | -- | yes | yes | yes | yes | yes | yes |
| Model cycling (Ctrl+P) | -- | yes | yes | yes | -- | yes | -- |
| OAuth flows | -- | yes | yes | yes | yes | yes | yes |
...
| Feature | tau | pi-mono | oh-my-pi | pi_agent_rust | codex | crush | opencode |
|---------|-----|---------|----------|---------------|-------|-------|----------|
| Extension API | -- | yes (TS) | yes (TS) | yes (JS/QuickJS + Rust + WASM) | yes (plugins) | -- | yes (TS plugin) |
| Skills (markdown) | -- | yes | yes | yes | yes | yes | yes |
| Custom tool registration | -- | yes | yes | yes | via MCP | -- | yes |
...
| Custom agents (markdown) | -- | -- | yes | -- | -- | -- | yes |
| MCP client | -- | stub | yes | stub | yes (stdio + HTTP) | yes | yes |
| MCP server mode | -- | -- | -- | -- | yes | -- | -- |
...
| Feature | tau | pi-mono | oh-my-pi | pi_agent_rust | codex | crush | opencode |
|---------|-----|---------|----------|---------------|-------|-------|----------|
| TUI framework | basic REPL | custom (diff renderer) | pi-tui | charmed (bubbletea port) | ratatui (Rust) | bubbletea | SolidJS (@opentui) |
...
| Diff view | -- | unified | unified | unified | yes (syntax-highlighted) | unified + split | -- |
| Clipboard paste (text+image) | -- | yes | yes | yes | yes (/copy) | -- | -- |
| External editor | -- | yes | -- | yes | yes ($VISUAL) | -- | -- |
| Autocomplete | -- | yes | yes | yes | yes (nucleo fuzzy) | yes | -- |
...
| Web UI | -- | yes (Lit) | -- | -- | yes (app-server + Electron) | -- | -- |
| IDE integration | -- | yes (RPC) | -- | yes (RPC) | yes (app-server) | -- | yes (ACP for Zed) |
...
| Feature | tau | pi-mono | oh-my-pi | pi_agent_rust | codex | crush | opencode |
|---------|-----|---------|----------|---------------|-------|-------|----------|
| Language | Rust | TypeScript | TypeScript | Rust | Rust + TypeScript | Go | TypeScript |
| Native addon | -- | -- | yes (N-API Rust) | -- | -- (pure Rust core) | -- | -- |
| SDK/embedding API | -- | yes | -- | yes | yes (TS SDK) | -- | -- |
...
3. **Sub-agent spawning** — oh-my-pi, codex, opencode have it natively; pi-mono via extension. Parallelism is the difference between "wait 5 minutes" and "wait 1 minute."
4. **MCP support** — oh-my-pi, codex, crush, opencode. Unlocks external tool servers without writing code.
...
6. **Web fetch/search** — oh-my-pi, codex, crush, opencode. Needed for looking up docs, APIs, error messages.
7. **Todo/plan tracking** — oh-my-pi, codex, crush, opencode. Keeps the agent organized on multi-step tasks.
8. **LSP diagnostics on edit** — oh-my-pi, crush, opencode. Immediate feedback on syntax/type errors after edits.
9. **Session picker / resume UX** — 5 harnesses have fuzzy pickers. tau has `--resume` but no browser.
...
16. **Multi-edit (batch)** — crush, opencode.
17. **Session branching/fork** — pi-mono, oh-my-pi, pi_agent_rust, codex.
